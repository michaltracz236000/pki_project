...
router.post('/', function (req, res, next) {
  ...
  text+='<nav class="navbar bg-primary"><div class="container-fluid"><span class="navbar-brand mb-0 h1 text-white">Witaj na stronie deleteRow</span></div></nav><div class="m-4">'
  const query = {
  ...
  pool.query(query)
    .then((result) => {
      ...
      res.send(text+'<h3 class="text-primary">Rekord został usunięty.</h3><form action="/showDB" method="POST"><button type="Submit" class="btn btn-secondary" name="db" value="' + req.body.tableName + '">Powrót</button></form></div></body></html>')
    })
    .catch((error) => {
